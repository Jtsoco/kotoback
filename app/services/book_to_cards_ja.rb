class BookToCardsJa < ApplicationJob
  ...
  def card_creator(book) #name pending
    ...
    epub_converter = EpubConverterJa.new(book)
    chapter_texts = epub_converter.call
    ...
    # p $title #using a global variable at the moment, need to test other alternatives
    # p checked_arrays # for test, remove afterward
    # raise # for test
...
# >>>>>>>>>>>>>>Unchanged original version that worked 
# def perform(book)
#   card_creator(book)
# end
# def card_creator(book) #name pending
...
# end
# end
# >>>>>>>>>>>>>>end of unchanged original version