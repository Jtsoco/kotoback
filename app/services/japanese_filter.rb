class JapaneseFilter
  ...
  def filter
    ...
    kanji_words = @singular_words.select do |word_hash|

     next false unless @kanji.match?(word_hash[:origin_word])
     next false if @filter_10000_common.include?(word_hash[:origin_word])

     true
    end
    kanji_words.tally.sort_by{|key, value| value}.first(50).to_h.keys

    # one word array for checking later
    # one_word_version = kanji_words.map {|hash| hash[:origin_word]}
    ...
    #   number = one_word_version.frequency(word)
    #   {
    #     origin_word: word,
    #     count: number
    #   }
